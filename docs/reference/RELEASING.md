...
- To repair reviewed native translations, configure the translation provider credentials and run `pnpm native:i18n:sync --locale <code> --refresh-id <native-id>`. Find IDs in `apps/.i18n/native-source.json`; repeat `--refresh-id` for up to 64 distinct IDs. The selector adds those entries to normal pending work, so missing strings and glossary changes still refresh as usual. Native translation requests include bounded nearby owner code as context; those excerpts are not stored in the source inventory. Unknown IDs are rejected before translation, and targeted refresh cannot be combined with `--force`. Then run `pnpm native:i18n:sync` to regenerate Android and Apple resources and `pnpm native:i18n:check` to validate the full result. Do not hand-edit generated translations.
