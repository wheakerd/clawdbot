...
- `doctor --fix` also updates drifted active official npm plugins from the OpenClaw catalog to the installed OpenClaw release, using the same plugin updater as `openclaw update`. It reports each outcome and rechecks restart readiness. A plugin that cannot be fetched remains a warning with the failure reason; other plugins can still be repaired and Doctor can finish. This repair leaves third-party plugins unchanged. The normal [Gateway maintenance and restart policy](/cli/doctor/running#postures) applies; follow the printed restart command when Doctor does not restart the Gateway for you.
