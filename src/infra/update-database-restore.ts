...
async function withDatabaseExclusion<T>(
  ...
  const acquire = async (index: number): Promise<T> => {
    ...
    return drainAgentDatabaseResources({ path: databasePath }, async () => {
...
export async function restoreUpdateDatabaseBackup(params: {
  ...
  const shared = resolvePathViaExistingAncestorSync(resolveOpenClawStateSqlitePath(params.env));
  const paths = [
    ...new Set([...backup.databases.map((entry) => entry.path), ...backup.missingPaths]),
  ].toSorted();
  const displaced: string[] = [];
  return await withDatabaseExclusion(shared, paths, assertCurrent, async (assertOwned) => {
    if (
