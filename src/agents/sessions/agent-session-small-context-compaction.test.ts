...
describe("AgentSession small-context compaction", () => {
  it.each([
    ...
    await expect(
      session[agentSessionAutomaticCompaction](undefined, "unresolved", undefined, budget),
    ...
    async (mode) => {
      ...
      const result =
        mode === "manual"
          ...
          : await session[agentSessionAutomaticCompaction](undefined, undefined, undefined, budget);
  ...
  it("reserves a recorder-persisted pending user once when retaining its exact row", async () => {
    ...
    await session[agentSessionAutomaticCompaction](undefined, "unresolved", undefined, budget);
