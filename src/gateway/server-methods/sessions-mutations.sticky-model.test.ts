...
describe("native runtime permission consent", () => {
  ...
  async function prepareConsent(entry: Partial<SessionEntry> = {}) {
    ...
    cfg.tools = { fs: { workspaceOnly: true }, deny: ["exec"] };
  ...
  it.each(["agent", "global"] as const)(
    ...
    async (selectionScope) => {
      ...
      cfg.agents!.defaults!.modelSelectionScope = selectionScope;
      const siblingScope = { ...fixture.scope, sessionKey: `${fixture.scope.sessionKey}:sibling` };
      ...
      expect(cfg.tools).toEqual({ fs: { workspaceOnly: true }, deny: ["exec"] });
