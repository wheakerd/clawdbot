...
describe("pinned pnpm cold bootstrap", () => {
  ...
  it("bootstraps from store, image, then registry while authenticating each archive", async ({
    ...
    for (const source of ["image", "store", "registry"]) {
      ...
      const result = await f.run();
      expect(result.status, result.stderr).toBe(0);
      const root = path.join(result.stdout.trim(), "v1/pnpm/12.5.0");
  ...
  it("bootstraps from the warmed store while archive downloads are unavailable", async ({
    ...
    expect(fs.existsSync(f.calls)).toBe(false);
    expect(fs.readFileSync(path.join(warm.stdout.trim(), "v1/pnpm/12.5.0/pnpm"), "utf8")).toBe(
      "wrapper-fixture\n",
    );
  });

  it.for([
    { name: "pnpm-12.5.0.tgz", fallback: "registry" },
    { name: "exe.linux-x64-12.5.0.tgz", fallback: "registry" },
    { name: "pnpm-12.5.0.tgz", fallback: "image" },
    { name: "exe.linux-x64-12.5.0.tgz", fallback: "image" },
  ])(
  ...
  it.for(["pnpm-12.5.0.tgz", "exe.linux-x64-12.5.0.tgz"])(
  ...
  it("does not fetch an unrecognized version or changed packageManager integrity", async ({
    ...
    const f = createPnpmArchiveFixture(command);
    for (const spec of [f.spec.replace("12.5.0", "12.5.1"), f.spec.replace(/.$/u, "z")]) {
...
describe("pnpm version output owns its failure", () => {
  ...
  it.for([0, 42])("preserves the version probe exit status %s", async (status, { command }) => {
    ...
    const run = `pnpm() { if [ ${status} -eq 0 ]; then printf '12.5.0\\n'; else return ${status}; fi; }\n${step.run}`;
    ...
    expect(fs.readFileSync(output, "utf8")).toBe(status === 0 ? "pnpm-version=12.5.0\n" : "");
  });
});
